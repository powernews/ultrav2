...
<div class="tbl_head01 tbl_wrap">
	...
	<tr>
		...
		<td>
			...
			&nbsp;
			<label><input type="checkbox" name="wset[caption]" value="1"<?php echo get_checked('1', $wset['caption']);?>> 스몰 캡션</label>
		</td>
	</tr>
	<tr>
	...
	<?php for ($i=1; $i <= $wset['slider']; $i++) { //총 7개 ?>
		...
		<tr>
			...
			<td>
				...
				<div style="height:8px;"></div>
				<select name="wset[bg<?php echo $i;?>]">
					<?php 
						if(!$wset['bg'.$i]) $wset['bg'.$i] = 'black';
						echo apms_color_options($wset['bg'.$i]);
					?>
				</select>
				캡션 배경색
				&nbsp;
				<label><input type="checkbox" name="wset[trans<?php echo $i;?>]" value="1"<?php echo get_checked('1', $wset['trans'.$i]);?>> 반투명 배경 적용</label>
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				<div style="height:8px;"></div>
				<label><input type="checkbox" name="wset[win<?php echo $i;?>]" value="1"<?php echo get_checked('1', $wset['win'.$i]);?>> 새창으로 링크 열기</label>
			</td>
		</tr>
		<tr>
